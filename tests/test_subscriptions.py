...
class TestSubscribeEndpoints:
    def _create_prompt(self, client, slug="sub-test"):
        return client.post("/api/v1/prompts", json={
            "slug": slug, "name": "Sub Test", "type": "persona",
        })
    ...
    def test_unsubscribe(self, client):
        self._create_prompt(client)
        client.post("/api/v1/prompts/sub-test/subscribe", headers={"X-Agent-ID": "agent-1"})
        resp = client.delete("/api/v1/prompts/sub-test/subscribe", headers={"X-Agent-ID": "agent-1"})
...
class TestAutoSubscribe:
    def test_auto_subscribe_on_version_fetch(self, client, sample_content):
        client.post("/api/v1/prompts", json={
            "slug": "auto-sub", "name": "AS", "type": "persona",
        })
        client.post("/api/v1/prompts/auto-sub/versions", json={
            "content": sample_content, "message": "v1",
        })
    ...
    def test_no_auto_subscribe_without_header(self, client, sample_content):
        client.post("/api/v1/prompts", json={
            "slug": "no-auto", "name": "NA", "type": "persona",
        })
        client.post("/api/v1/prompts/no-auto/versions", json={
            "content": sample_content, "message": "v1",
        })
...
class TestEventPublishing:
    def test_version_create_notifies_subscribers(self, client, mock_db, sample_content):
        client.post("/api/v1/prompts", json={
            "slug": "evented", "name": "E", "type": "persona",
        })
        # Subscribe an agent
        ...
        with patch("prompt_forge.core.events.get_event_publisher") as mock_pub:
            ...
            client.post("/api/v1/prompts/evented/versions", json={
                "content": sample_content, "message": "update", "priority": "critical",
            })

            # Check publish was called with correct subject
            if publisher.publish.called:
                call_args = publisher.publish.call_args
                assert "agent-notify" in call_args.kwargs.get("subject", call_args[1].get("subject", ""))
                assert call_args.kwargs.get("data", call_args[1].get("data", {})).get("priority") == "critical"
...
class TestTTLCleanup:
    def test_stale_subscriptions_removed(self, mock_db):
        ...
        mock_db.insert("prompt_subscriptions", {
            "prompt_id": "fake-id",
...
class TestSubscriberCount:
    def test_list_prompts_includes_subscriber_count(self, client):
        client.post("/api/v1/prompts", json={
            "slug": "counted", "name": "C", "type": "persona",
        })
