...
async def branch_diff(
    ...
    try:
        ...
        if not branch_history:
            raise HTTPException(status_code=404, detail=f"No versions found on branch '{branch_name}'")
...
def _generate_diff_summary(current: dict[str, Any], proposed: dict[str, Any]) -> str:
    ...
    for section_id in set(current_sections.keys()) & set(proposed_sections.keys()):
        if current_sections[section_id].get("content") != proposed_sections[section_id].get("content"):
            modified_sections.append(section_id)
    
    if modified_sections:
        changes.append(f"Modified {len(modified_sections)} section(s): {', '.join(modified_sections)}")
    ...
    modified_vars = [k for k in set(current_vars.keys()) & set(proposed_vars.keys()) 
